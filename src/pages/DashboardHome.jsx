...
import { Notifications, Star, Article, School, MenuBook, Home, Person, CalendarToday } from 'lucide-react';

const DashboardHome = () => {
  const [activeTab, setActiveTab] = useState('lessons');

  return (
    <div className="bg-background-canvas text-on-surface min-h-screen pb-32">
      {/* Top App Bar */}
      <header className="flex justify-between items-center px-container-margin py-4 w-full bg-background dark:bg-background">
        ...
        <button className="w-10 h-10 flex items-center justify-center hover:bg-surface-container-high rounded-full transition-colors active:scale-95">
          <Notifications size={24} className="text-primary" />
        </button>
      </header>
      ...
      <main className="px-container-margin">
        ...
        <nav className="flex gap-2 overflow-x-auto py-4 no-scrollbar">
          <button
            className={`px-6 py-2.5 rounded-full text-label-sm font-label-sm ${activeTab === 'forum' ? 'bg-primary text-white' : 'bg-surface-container-highest text-on-surface-variant hover:bg-surface-container-high transition-colors'}`}
            onClick={() => setActiveTab('forum')}
          >
            Forum
          </button>
          <button
            className={`px-6 py-2.5 rounded-full text-label-sm font-label-sm ${activeTab === 'add-classes' ? 'bg-primary text-white' : 'bg-surface-container-highest text-on-surface-variant hover:bg-surface-container-high transition-colors'}`}
            onClick={() => setActiveTab('add-classes')}
          >
            Add. classes
          </button>
          <button
            className={`px-6 py-2.5 rounded-full text-label-sm font-label-sm ${activeTab === 'lessons' ? 'bg-primary text-white shadow-md shadow-primary/20 transition-all scale-105' : 'bg-surface-container-highest text-on-surface-variant hover:bg-surface-container-high transition-colors'}`}
        ...
        <div className="grid grid-cols-2 gap-4 mt-2">
          ...
          <div className="bg-secondary-container rounded-3xl p-card-padding flex flex-col justify-between aspect-[4/5] relative overflow-hidden group active:scale-[0.98] transition-transform">
            ...
            <div className="flex justify-between items-start">
              <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                <Article size={20} className="text-white" />
              </div>
            </div>
            <div>
              ...
              <div className="mt-4 w-8 h-8 rounded-full overflow-hidden border border-white/30">
                <img className="w-full h-full object-cover" alt="A portrait of teacher Amy Adams, radiating wisdom and enthusiasm in a modern academic setting. The shot uses soft natural lighting to create an inviting atmosphere. The overall style is clean and corporate-modern, utilizing warm coral and white tones to match the subject's theme. High-end photography with shallow depth of field." src="https://lh3.googleusercontent.com/aida-public/AB6AXuBDX690UKVtjMnIxfuHsXyoyu0YahpRbozwZa8L2UEJ-w44Kd-us7nzFOkN_uoMQiFQrgzH34IhRNR8NpLHwlEkN__KE0aOTGO2Owzitinc3vS8thJugGnqz8HoouWxL_iss7IK3U-uJ4bVnGHp0tUfuM95dlta9mLIrUFb-5jOr2AFAW3gmSw7hlJM5kGH6lrConDdFjk5y3rEVeqtRh2BVRBd6gstkqvBpeRs5Nod42jWWhcbts34Ug"/>
              </div>
            </div>
          </div>
          ...
          <div className="bg-tertiary-yellow rounded-3xl p-card-padding flex flex-col justify-between aspect-[4/5] relative overflow-hidden group active:scale-[0.98] transition-transform">
            ...
            <div className="flex justify-between items-start">
              <div className="w-10 h-10 bg-black/10 rounded-xl flex items-center justify-center">
                <School size={20} className="text-on-tertiary-fixed" />
              </div>
            </div>
            <div>
              ...
              <div className="mt-4 w-8 h-8 rounded-full overflow-hidden border border-black/10">
                <img className="w-full h-full object-cover" alt="Portrait of math teacher Amy Johnson, looking smart and encouraging. She is set against a bright, minimalist modern background that reflects a high-energy educational environment. The lighting is crisp and clear, highlighting a professional corporate look with vibrant yellow and neutral grey accents. Sophisticated and high-quality digital photography." src="https://lh3.googleusercontent.com/aida-public/AB6AXuAAkj09cpe3G8aYPk5yQsRkdwNfWVfjqxaqvI9aeTOv5dLmVUG6hWDGZDm9wpUPvcz8BkJd0W0ZaeYSikeZ6jhDeWXOtw044j2R9X8Hci0Iwlgc53oyO1D1a3wC6fsiPD7TRi4Jojex4hBSR7GUzZ2OsMFvepIk-qu7y5heRkhkV42U9URWuam_4jgVlIjMZwoaWCKgquegE9c7KAWbOCHlR-IHp9XfuwjaOkphNdKOJ-2LxVLa8uOUxQ"/>
              </div>
            </div>
          </div>
          ...
          <div className="bg-quaternary-green rounded-3xl p-card-padding flex flex-col justify-between aspect-[4/5] relative overflow-hidden group active:scale-[0.98] transition-transform">
            ...
            <div className="flex justify-between items-start">
              <div className="w-10 h-10 bg-white/30 rounded-xl flex items-center justify-center">
                <MenuBook size={20} className="text-white" />
              </div>
            </div>
            <div>
        ...
        <div className="mt-4 p-card-padding bg-surface-container-lowest rounded-3xl flex items-center justify-between border border-outline-variant/30">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center">
              <MenuBook size={24} className="text-primary" />
            </div>
            <div>
      ...
      <nav className="fixed bottom-6 left-0 right-0 z-50 flex justify-around items-center px-4 py-3 mx-auto max-w-md bg-primary-container dark:bg-primary-container shadow-lg shadow-primary/20 rounded-full w-[90%]">
        <Link to="/" className="flex items-center justify-center bg-surface-container-lowest dark:bg-surface-container-lowest text-primary dark:text-primary rounded-full p-3 active:scale-90 transition-transform duration-200">
          <Home size={24} />
        </Link>
        <Link to="/homework" className="flex items-center justify-center text-on-primary-container/70 dark:text-on-primary-container/70 p-3 hover:bg-primary/10 rounded-full transition-all active:scale-90 duration-200">
          <MenuBook size={24} />
        </Link>
        <Link to="/calendar" className="flex items-center justify-center text-on-primary-container/70 dark:text-on-primary-container/70 p-3 hover:bg-primary/10 rounded-full transition-all active:scale-90 duration-200">
          <CalendarToday size={24} />
        </Link>
        <Link to="/profile" className="flex items-center justify-center text-on-primary-container/70 dark:text-on-primary-container/70 p-3 hover:bg-primary/10 rounded-full transition-all active:scale-90 duration-200">
          <Person size={24} />
        </Link>
      </nav>
    </div>
